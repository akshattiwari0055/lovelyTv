...
export default defineConfig({
  plugins: [
    react(),
    {
      ...
      enforce: "pre",
      transform(code, id) {
        if (!id.includes("zego-uikit-prebuilt")) return null;
        ...
        patched = patched.replace(/(\w+)\.end\(\)/g, (_match, varName) => `${varName}?.end()`);
      ...
      plugins: [
        {
          ...
          setup(build) {
            build.onLoad({ filter: /zego-uikit-prebuilt/ }, (args) => {
              ...
              code = code.replace(/(\w+)\.end\(\)/g, (_, v) => `${v}?.end()`);
